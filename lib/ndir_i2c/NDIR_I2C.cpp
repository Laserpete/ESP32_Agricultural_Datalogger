...
uint8_t NDIR_I2C::cmd_measure[9] = {0xFF, 0x01, 0x9C, 0x00, 0x00,
                                    0x00, 0x00, 0x00, 0x63};
uint8_t NDIR_I2C::cmd_calibrateZero[9] = {0xFF, 0x01, 0x87, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x78};
uint8_t NDIR_I2C::cmd_enableAutoCalibration[9] = {0xFF, 0x01, 0x79, 0xA0, 0x00,
                                                  0x00, 0x00, 0x00, 0xE6};
uint8_t NDIR_I2C::cmd_disableAutoCalibration[9] = {0xFF, 0x01, 0x79, 0x00, 0x00,
                                                   0x00, 0x00, 0x00, 0x86};
...
uint8_t NDIR_I2C::parse(uint8_t *pbuf) {
  ...
  if (pbuf[0] == 0xFF && pbuf[1] == 0x9C && checksum == 0xFF) {
    ppm = (uint32_t)pbuf[2] << 24 | (uint32_t)pbuf[3] << 16 |
          (uint32_t)pbuf[4] << 8 | pbuf[5];
    return true;
  } else {
    return false;
  }
